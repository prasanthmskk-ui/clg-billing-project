...
const TAMIL_PHONETIC_VARIANTS = ['pal', 'paal', 'paul', 'pall', 'பால்']
...
export function mapVoiceProduct(text) {
  ...
  const match = Object.entries(VOICE_PRODUCT_MAP).find(([key]) => normalized.includes(key))
  return match ? match[1] : String(text || '').trim()
}

export function mapTamilPhonetic(text) {
  const normalized = String(text || '').trim().toLowerCase().replace(/\s+/g, ' ')
  return TAMIL_PHONETIC_VARIANTS.some((variant) => normalized.includes(variant)) ? 'பால்' : null
}
